...
const LEGACY_STAFF_ROUTE_REDIRECTS = {
  "/staff/appointments": "/staff/bookings",
  "/staff/reminders": "/staff/follow-ups",
  "/staff/patients": "/staff/customers",
} as const;

function canonicalStaffRoute(pathname: string): string | null {
  return LEGACY_STAFF_ROUTE_REDIRECTS[
    pathname as keyof typeof LEGACY_STAFF_ROUTE_REDIRECTS
  ] ?? null;
}
...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  beforeLoad: ({ location }) => {
    const target = canonicalStaffRoute(location.pathname);
    if (target) {
      // routeTree.gen.ts is regenerated by TanStack Router; the cast only keeps the
      // checked-in stale generated tree from blocking this compatibility redirect.
      throw redirect({ to: target as "/staff/today", replace: true });
    }
  },
  head: () => ({
    meta: [
